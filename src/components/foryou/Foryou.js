...
const ForYou = () => {
    ...
    useEffect(() => {
        const fetchKnots = async () => {
            setLoading(true)
            try {
                const res = await axios.get("/api/knot")
                setData(res.data)
            ...
            <div className="ForYou">
                {!loading &&
                    data.map((knot) => {
                        ...
                        return (
                            <div onClick={() => knotHandler(knot._id)} key={knot._id} className="box">
                                {/* upper section */}
                                <div className="upper">
                                    <div className="header-section">
                                        <div className="user-logo">
                                            <img onClick={(e) => {
                                                e.stopPropagation()
                                                profileHandler(knot.userId.username)
                                            }} src={knot.userId.avatar || "./user_logo.png"} alt="" />
                                        </div>
                                        <div className="user-info">
                                            <h1 onClick={(e) => {
                                                e.stopPropagation()
                                                profileHandler(knot.userId.username)
                                            }}>{knot.userId.username}</h1>
                                            ...
                                            {knot.images.length > 0 && (
                                                <div className="image-container">
                                                    {knot.images.map((img, idx) => (
                                                        <div onClick={(e) => {
                                                            e.stopPropagation()
                                                            handleImageClick(idx, data.indexOf(knot))
